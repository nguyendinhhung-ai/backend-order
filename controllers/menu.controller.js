...
// Lấy menu (KHÔNG lấy hidden)
exports.getMenu = (req, res) => {
    const sql = "SELECT * FROM menu WHERE status = 'available' ORDER BY id DESC";
...
exports.updateMenuItem = (req, res) => {
    ...
    db.query(sql, [name, price, image || "", status || "available", id], (err) => {
        if (err)
            return res.status(500).json({ message: "Lỗi cập nhật món", error: err });
